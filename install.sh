...
NETWORK_CMD="$INSTALL_DIR/network.sh"
...
echo "⬇️  network.sh"
curl -fsSL "$RAW_BASE/network.sh" -o "$NETWORK_CMD"
chmod +x "$NETWORK_CMD"
...
INSTALL_DIR="$HOME/Documents/alias"
mkdir -p "$INSTALL_DIR"
for f in search.sh network.sh update.sh; do
...
ADD_ALIAS_NETWORK='alias network="bash ~/Documents/alias/network.sh"'
...
for SHELLRC in "$HOME/.bashrc" "$HOME/.zshrc"; do
  ...
  grep -qF "$ADD_ALIAS_NETWORK" "$SHELLRC" || echo "$ADD_ALIAS_NETWORK" >> "$SHELLRC"
